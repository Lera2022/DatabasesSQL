...
SELECT login FROM users UNION SELECT login FROM clients;

SELECT login FROM users UNION ALL SELECT login FROM clients;
...
# Получить столбцы из таблицы “clients” , в которых первое имя является набором значений ('Mikle', 'Tom', 'Masha').
SELECT login FROM clients WHERE login in ('Mikle', 'Tom', 'Masha');

# Выберите все логины из таблицы “users”, кроме “Mikle”.
SELECT login FROM users WHERE login != 'Mikle';

SELECT * FROM Employee;

SELECT * FROM Projects;

SELECT * FROM Employee WHERE EXISTS (SELECT * FROM Projects WHERE Employee.Id = Projects.EmployeeId);
